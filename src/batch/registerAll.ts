...
import registerCreatorInDB from "../channel/register";
...
const registerAll = async () => {
  ...
  logger.info(`registerAll: Registering ${new_slugs.length} creators`);
  ...
  for (const slug of new_slugs) {
    ...
    try {
      ...
      logger.error(error);
      logger.error("registerAll: Failed to register creator", { slug });
    } finally {
      ...
      logger.info(`registerAll: ${remaining_slugs} creators remaining`);
